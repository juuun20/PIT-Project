...
if(!empty($_POST['save'])) {
    ...
    if($p2 == $p3) {
        ...
        $stmt = $con->prepare("SELECT * FROM staff WHERE password=?");
        ...
        if($count > 0) {
            ...
            $stmt = $con->prepare("UPDATE staff SET password=? WHERE password=?");
...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="STYLES/change password-staff.css">
