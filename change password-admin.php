...
if(!empty($_POST['save'])) {
    ...
    if($p2 == $p3) {
        ...
        $stmt = $con->prepare("SELECT * FROM admin WHERE password=?");
        ...
        if($count > 0) {
            ...
            $stmt = $con->prepare("UPDATE admin SET password=? WHERE password=?");
...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="STYLES/change password-admin.css">
